...
func deleteLogsBeforeHandler(c *fiber.Ctx) error {
	...
	// 记录操作日志
	adminlog.WriteLog(c, "delete", "gamelog", 0, fmt.Sprintf("批量删除%s之前的游戏日志，共%d条", dateStr, result.RowsAffected))
...
func deleteLogHandler(c *fiber.Ctx) error {
	...
	// 记录操作日志
	adminlog.WriteLog(c, "delete", "gamelog", uint(log.ID), fmt.Sprintf("删除游戏日志，角色：%s", log.RoleName))
